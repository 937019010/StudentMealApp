...
import com.mealapp.domain.usecase.StudentManagementUseCase
...
class StudentViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val students: StateFlow<List<Student>> = repository.allStudents
        .map { entities -> entities.map { Student(it.id, it.name, it.orderIndex) } }
    ...
    fun addStudents(names: List<String>) {
        viewModelScope.launch {
    ...
    fun updateStudent(id: Long, newName: String) {
        viewModelScope.launch {
            if (newName.isBlank()) {
                _operationResult.emit(OperationResult.Error("姓名不能为空"))
                return@launch
    ...
    fun reorderStudents(studentIds: List<Long>) {
        viewModelScope.launch {
